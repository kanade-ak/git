...
test_description='git.kanade.one remote whitelist'
...
test_expect_success 'clone is not restricted by the git.kanade.one whitelist' '
	rm -rf clone-helper &&
...
test_expect_success 'git.kanade.one SSH-style remotes are accepted by URL parser' '
	git fetch-pack --diag-url git@git.kanade.one:repo >actual &&
	test_grep "userandhost=git@git.kanade.one" actual
'

test_expect_success 'git.kanade.one subdomains are rejected' '
	test_must_fail git fetch-pack --diag-url git@mirror.git.kanade.one:repo 2>err &&
	test_grep "only git.kanade.one remotes are allowed" err
'
